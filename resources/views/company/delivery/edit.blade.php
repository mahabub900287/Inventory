<x-admin.layouts.app :title="getbreadcumb()">
    ...
    <div class="ic-home-content">
        ...
        <form action="{{ route('company.delivery.update', $delivery->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                                        ...
                                        <div class="d-flex mb_30 ic_form">
                                            <div class="form-check me-3">
                                                <input class="form-check-input" type="radio" name="product_type"
                                                    id="product_type1" value="0"
                                                    @if ($delivery->product_type == 0) checked @endif>
                                            ...
                                            <div class="form-check ms-3">
                                                <input class="form-check-input" type="radio" name="product_type"
                                                    ...
                                                    @if ($delivery->product_type == 1) checked @endif>
                                                        ...
                                                        <div class="col-12 form-check-wrapper">
                                                            <div class="form-check" data-bs-toggle="tooltip"
                                                                ...
                                                                <input type="checkbox" class="form-check-input"
                                                                    ...
                                                                    @if ($delivery->inbound) checked @endif>
                                                ...
                                                <div class="mb_20">
                                                    ...
                                                    <select class="ic-select2" name="warehouse_id">
                                                        @foreach ($warehouses as $warehouse)
                                                            <option value="{{ $warehouse->id }}" data-badge=""
                                                                @if ($warehouse->id == $delivery->warehouse_id) selected @endif>
                                            ...
                                            <div class="col-md-6">
                                                <div class="mb_20">
                                                    <input type="hidden" name="delivery_type" class="form-control"
                                                        id="package_type" value="{{ $delivery->delivery_type }}">
                                                    <x-validation-error-message name="delivery_type" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row ic_form">
                                            ...
                                            <div class="col-md-6">
                                                <div class="mb_20">
                                                    ...
                                                    <input type="date" class="form-control" name="start_date"
                                                        value="{{ date('Y-m-d', strtotime($delivery->start_date)) }}"
                                            ...
                                            <div class="col-md-6">
                                                <div class="mb_20">
                                                    ...
                                                    <input type="date" class="form-control" name="end_date"
                                                        value="{{ date('Y-m-d', strtotime($delivery->end_date)) }}"
                                                        placeholder="Alphanumeric code">
                                                    <span class="err-txt"></span>
    ...
    <x-slot name="bottomScript">
        <script>
            jQuery(document).ready(function() {
                var products = {!! json_encode(
                    count($delivery->deliveryProducts) > 0 ? $delivery->deliveryProducts : $delivery->deliveryBundles,
                ) !!};
                ...
                for (let index = 0; index < products.length; index++) {
                    var productName;
                    var productId;
                    if ({{ $delivery->product_type }} === 0) {
                        productName = products[index].products.name;
                        productId = products[index].products.id;
                    } else {
                        productName = products[index].bundles.name;
                        productId = products[index].bundles.id;
                    }

                    var appendHTML = '<div class="ic-add-parcel-pallet-wrapper mb_20 ic_form input-wrapper">' +
                        '<div class="ic-add-parcel-pallet-single-item">' + '<label class="form-label">' + productName +
                        '</label>' +
                        '<input type="hidden" class="form-control" name="product_id[]" value="' + productId + '">' +
                        '</div>' +
                        '<div class="ic-add-parcel-pallet-single-item">' +
                        '<label class="form-label">Quantity</label>' +
                        '<input type="text" class="form-control" value="' + products[index].quantity +
                        '" name="product_qty[]">' +
                        '<span class="err-txt"></span>' + '</div>' + '<div class="ic-add-parcel-pallet-single-item">' +
                        '<label class="form-label">Box Number</label>' +
                        '<input type="text" class="form-control" name="product_tracking[]" value="' + products[index]
                        .tracking_number +
                        '">' + '<span class="err-txt"></span>' + '</div>' +
                        '<div class="ic-add-parcel-pallet-single-item">' +
                        '<label class="form-label" style="visibility: hidden; opacity: 0;">delete</label>' +
                        '<button class="ic-delete ic-common-border-icon" id="remove-product">' +
                        '<i class="ri-delete-bin-6-line"></i>' +
                        '</button>' + '</div>' + '</div>';
                    $('.product-wrapper').append(appendHTML);
                }
                var maxLimit = 20;
                var x = {{ count($delivery->deliveryProducts) }};
        ...
        <script>
            jQuery(document).ready(function() {
                var parcels = {!! json_encode($delivery->delivery_metarial) !!};
                var parcelData = JSON.parse(parcels)

                for (let index = 0; index < parcelData.length; index++) {
                    const element = parcelData[index];
                    var appendHTML =
                        ...
                        element.parcel_qty +
                        '" name="parcel_qty[]"><span class="err-txt"></span></div><div class="ic-add-parcel-pallet-single-item"><label class="form-label">Tracking Number</label><input type="text" class="form-control" name="parcel_tracking[]" value="' +
                        element.parcel_tracking +
                        '"><span class="err-txt"></span></div><div class="ic-add-parcel-pallet-single-item"><label class="form-label" style="visibility : 0; opacity : 0">delete</label><button class="ic-delete ic-common-border-icon" id="remove-parcel"><i class="ri-delete-bin-6-line"></i></button></div></div>';
                    $('#parcels-wrapper').append(appendHTML);
                }
                var maxLimit = 20;
                var x = parcelData.length;
            ...
            var type = {{ $delivery->product_type }};
            $.ajax({
                type: "POST",
                url: "{{ route('ajax-get-product-bundle') }}",
                data: {
