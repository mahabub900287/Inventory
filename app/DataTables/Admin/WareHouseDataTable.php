...
class WareHouseDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                ...
                return '<div class="ic-action-wrapper">' .
                    (auth()->user()->can('Edit Ware-House') ?
                        '<div class="ic-action">
                            <a href="' . route('admin.ware-house.edit', $item->id) . '"><i class="ri-pencil-line"></i></a>
                        </div>' : '') .
                    (auth()->user()->can('Delete Ware-House') ?
                        '<div class="ic-action">
                        <form action="' . route('admin.ware-house.destroy', $item->id) . '"  id="delete-form-' . $item->id . '" method="post" style="">
            ...
            ->rawColumns(['action', 'user_id', 'status', 'checkbox'])->addIndexColumn();
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('serial_code', 'serial_code')->title('Serial Number'),
            Column::make('user_id', 'user_id')->title('Contact Person'),
